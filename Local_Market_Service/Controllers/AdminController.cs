...
namespace Local_Market_Service.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        private void RemoveCustomerRelatedData(Customer customer)
        {
            var bookings = context.Booking.Where(b => b.CustomerId == customer.Id).ToList();
            var bookingIds = bookings.Select(b => b.Id).ToList();

            context.Carts.RemoveRange(context.Carts.Where(c => c.CustomerId == customer.Id));
            context.Payments.RemoveRange(context.Payments.Where(p => bookingIds.Contains(p.BookingId)));
            context.Reviews.RemoveRange(context.Reviews.Where(r => r.CustomerId == customer.Id || bookingIds.Contains(r.BookingId)));
        ...
        public IActionResult DeleteCustomer(string id)
        {
            ...
            if (user != null)
            {
                ...
                if (customer != null)
                {
                    RemoveCustomerRelatedData(customer);
        ...
        private void RemoveProviderRelatedData(Provider provider)
        {
            var services = context.Services.Where(s => s.ProviderId == provider.Id).ToList();
            var serviceIds = services.Select(s => s.Id).ToList();
            var bookings = context.Booking.Where(b => b.ProviderId == provider.Id || serviceIds.Contains(b.ServiceId)).ToList();
            var bookingIds = bookings.Select(b => b.Id).ToList();

            context.Carts.RemoveRange(context.Carts.Where(c => serviceIds.Contains(c.ServiceId)));
            context.Payments.RemoveRange(context.Payments.Where(p => bookingIds.Contains(p.BookingId)));
            context.Reviews.RemoveRange(context.Reviews.Where(r => r.ProviderId == provider.Id || bookingIds.Contains(r.BookingId)));
            context.Booking.RemoveRange(bookings);
            context.Services.RemoveRange(services);
            context.ProviderDocuments.RemoveRange(context.ProviderDocuments.Where(d => d.ProviderId == provider.Id));
        }
        ...
        public IActionResult DeleteProvider(int id)
        {
            ...
            if (provider != null)
            {
                RemoveProviderRelatedData(provider);
        ...
        private void RemoveCategoryRelatedData(Category category)
        {
            var services = context.Services.Where(s => s.CategoryId == category.Id).ToList();
            var serviceIds = services.Select(s => s.Id).ToList();
            var bookings = context.Booking.Where(b => serviceIds.Contains(b.ServiceId)).ToList();
            var bookingIds = bookings.Select(b => b.Id).ToList();

            context.Carts.RemoveRange(context.Carts.Where(c => serviceIds.Contains(c.ServiceId)));
            context.Payments.RemoveRange(context.Payments.Where(p => bookingIds.Contains(p.BookingId)));
            context.Reviews.RemoveRange(context.Reviews.Where(r => bookingIds.Contains(r.BookingId)));
            context.Booking.RemoveRange(bookings);
            context.Services.RemoveRange(services);
        }
